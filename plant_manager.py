...
def update_plant(plant_id: int, plant_data: dict) -> bool:
    ...
    Args:
        ...
        plant_data: A dictionary containing the fields to update and their
                    new values.
...
if __name__ == '__main__':
    ...
    # 2. Get all plants
    print("\n--- All Plants ---")
    all_plants_list = get_all_plants()
    if all_plants_list:
        for plant_item in all_plants_list:
            print(
                f"- ID: {plant_item['plant_id']}, "
                f"Name: {plant_item['name']}, "
                f"Species: {plant_item['species']}"
            )
    else:
    ...
    if plant1_id:
        ...
        plant_detail_info = get_plant(plant1_id)
        if plant_detail_info:
            for key, value in plant_detail_info.items():
                print(f"  {key}: {value}")
        else:
    ...
    if plant1_id:
        update_data_dict = {
            'notes': 'Actually quite resilient.',
            'current_symptoms': 'Slightly yellowing leaves'
        }
        print(f"\n--- Updating Plant ID {plant1_id} ---")
        if update_plant(plant1_id, update_data_dict):
            ...
            if updated_plant_detail:
                ...
                print(
                    f"  New symptoms: "
                    f"{updated_plant_detail['current_symptoms']}"
                )
        else:
    ...
    all_plants_after_update_list = get_all_plants()
    if all_plants_after_update_list:
        for plant_item_updated in all_plants_after_update_list:
            print(
                f"- ID: {plant_item_updated['plant_id']}, "
                f"Name: {plant_item_updated['name']}, "
                f"Notes: {plant_item_updated.get('notes')}"
            )
    else:
        print("No plants found or error retrieving them.")
    ...
    all_plants_after_deletion_list = get_all_plants()
    if all_plants_after_deletion_list:
        for plant_item_after_delete in all_plants_after_deletion_list:
            print(
                f"- ID: {plant_item_after_delete['plant_id']}, "
                f"Name: {plant_item_after_delete['name']}"
            )
    else:
        print("No plants found or error retrieving them.")
